...
targets_dir = 'choices'
results_dir = os.path.join('two_region_results', 'run_transparent')
		...
		for c_slice in os.listdir(target_infile_path):
			...
			with open(os.path.join(slice_path,'console_readout.txt'), 'w') as f:
				...
				if search_size > min(pix.shape):
					...
					s = r.Subslice(slicePic,prob,target,useTransparentPix=useTransparentPix,size=min(pix.shape)-1,hn=hn)
				else:
					s = r.Subslice(slicePic,prob,target,useTransparentPix=useTransparentPix,template=template)
				...
				drawBoxes(sliceIm,boxes,slice_path,'sp_regions.jpg')
...
def writeResults(choices,s,s_flipped,f,path):
	f.write('\nComposite Results:\n\n')
	writeSubResults(choices,s,s_flipped,'composite',f,path)

	f.write('\nPrimary Results:\n\n')
	writeSubResults(choices,s,s_flipped,'primary',f,path)

	f.write('\nSecondary Results:\n\n')
	writeSubResults(choices,s,s_flipped,'secondary',f,path)
...
def writeSubResults(choices,s,s_flipped,result_type,f,path):
	...
	for c_index, choice in enumerate(choices):
		...
		p_ret = s_flipped.primary.ret.copy() if flipped else s.primary.ret.copy()
		...
		f.write('\t\t\t-Similarity: %f\n' % res['s'].sim)
		s_ret = s_flipped.secondary.ret.copy() if flipped else s.secondary.ret.copy()
