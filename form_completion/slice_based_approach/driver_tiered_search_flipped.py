...
targets_dir = 'choices'
results_dir = os.path.join('tiered_results', 'run1')
	...
	for target in os.listdir(target_path):
		...
		for c_slice in os.listdir(target_infile_path):
			...
			with open(os.path.join(slice_path,'console_readout.txt'), 'w') as f:
				...
				s_small = r.Slice(slicePic_small,prob,target,hn=hn_coarse)
				s = r.Slice(slicePic,prob,target,hn=hn_refined)
...
def primarySearch(choices,s,locations):
	...
	for c_index in range(len(choices)):
		s.results.append(FlipResult((0,0),0,False,0))
		s_flipped.results.append(FlipResult((0,0),0,True,0))
